...
class ActiveMQMessageConverter {
	...
	public static Message convertMessage(Session session, javax.jms.Message in) throws JMSException {
		...
		// If the message is already an ActiveMQ message, return it.
		if(org.apache.activemq.command.ActiveMQMessage.class.isAssignableFrom(in.getClass()))
			return in;
		
		// The message is from a foreign JMS provider, convert it into an ActiveMQ message.
