...
rem use shiped JRE if available, otherwise fallback to JAVA_HOME and PATH
...
if "%~1"=="debug" (
    call %LAUNCHER_JAVA_DEBUG% %JVM_OPTS% -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005 -p bootstrap\update4j.jar --add-modules jdk.unsupported,java.scripting -m org.update4j/org.update4j.Bootstrap --remote %UPDATE_URL% --syncLocal --local bootstrap.xml --launchFirst
) else (
